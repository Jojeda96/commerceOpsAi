...
class NLPEngine:
    ...
    def search_reviews(self, query: str, top_k: int = 5) -> Dict[str, Any]:
        if self.embeddings is not None and len(self.reviews) > 0:
            try:
                if self.model is not None:
                    ...
                    # Fallback simple de coincidencia léxica / coincidencia de tokens
                    query_words = set(query.lower().split())
                    scores = []
                    for r in self.reviews:
                        text = r["review_comment_message"].lower()
                ...
                for idx in top_indices:
                    rev = self.reviews[idx]
                    sim = float(scores[idx])
                    results.append({
                        "review_id": rev.get("review_id", f"rev-{idx}"),
                ...
                return {
                    "query": query,
                    "results": results,
                    "method": "all-MiniLM-L6-v2_cosine_similarity" if self.model else "tfidf_lexical_similarity",
                }
            except Exception as e:
